...
function getNumDays(monthIndex) {
  ...
  var TotalDays = 0; //Counter for total number of days

  for (k = 0; k < monthIndex; k++) { //For loop to count based on monthIndex
    TotalDays = TotalDays + getDaysInMonth(k); //Add to TotalDays based on the days in that month
  }

  return TotalDays; //returning the total number of days
}
...
function getDaysInMonth(month) {
  if (month == 1) { //February has 29 days
    return 29;
  }
  else if (month == 3 || month == 5 || month == 8 || month == 10) { //April, June, September, and November have 30 days
    return 30;
  } 
